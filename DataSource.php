...
class DataSource {
    ...
    public function getStatus() : string {
        ...
        return "Currently connected to $source!";
    }
    ...
    public function getMovieManager() : MovieManagerInterface {
        ...
        if ($this->getSource() === 'file'){
            ...
            $path = "$document_root/../PHPassignment3/list.txt";
            $manager = new FileMovieManager($path);
        } else if ($this->getSource() === 'database') {
            $manager = new DatabaseMovieManager('localhost', 'user', 'Aura_Storm15', 'movie_log');
        }
        return $manager;
    }
...
EOT;
    ...
    private function listFromFile(string $data) : string
    {
        ...
        foreach ($movies as $key => $entry) {
            ...
            $table_body .= '<td><a href="edit.php?id=' . $key . '&source=' . $this->getSource() . '" class="btn btn-info btn-sm">Edit</a></td>';
            $table_body .= '</tr>';
        }
        return $table_body;
    }
